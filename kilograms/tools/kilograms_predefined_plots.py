...
def main():
    parser = argparse.ArgumentParser(
        description="Produce a set of predefined kilograms plots."
    )
    parser.add_argument(
        "data_path", help="File containing data to be plotted", type=Path
    )
    parser.add_argument(
        "-c",
        "--config",
        help="Config file, containing plot type definitions.",
        type=Path,
        required=True,
    )
    parser.add_argument(
        "-n",
        "--names",
        help="Names of plots to be produced (comma-separated)",
        type=str,
        default=None,
    )
    parser.add_argument(
        "-o",
        "--output",
        help='Output file path. Will display onscreen if omitted. "?" will be replaced by plot name (remember to bash-escape it).',
        type=str,
        default="plot-?.pdf",
    )
    parser.add_argument(
